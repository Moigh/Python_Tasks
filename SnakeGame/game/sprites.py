...
class SnakeSegment(pygame.sprite.Sprite):
    def __init__(self, segment_type, direction1, direction2,
                 pos_x, pos_y, field_x, field_y, cell_size):
        super().__init__()
        ...
        if segment_type == 'head':
            ...
            # Для поворота определяем правильное направление на основе двух
            # направлений
...
class Snake:
    ...
    def move(self):
        ...
        if new_head_x < 0:
            new_head_x = self.cells_x - 1
        elif new_head_x >= self.cells_x:
            new_head_x = 0
        if new_head_y < 0:
            new_head_y = self.cells_y - 1
